...
def process_email(gmail_client: GmailClient, ai_engine: AIEngine, email: Dict):
    """Process a single email"""
    try:
        logger.info(f"Processing email: {email['subject']}")
        
        # Generate AI response and possibly an image
        response, image_path = ai_engine.generate_response(email)
...
def process_emails(gmail_client: GmailClient, ai_engine: AIEngine):
    ...
    try:
        ...
        for email in emails:
            process_email(gmail_client, ai_engine, email)
...
def main():
    ...
    try:
        # Setup application
        max_emails, response_delay = setup()
        ...
        logger.info(f"Starting email monitor for: {TARGET_EMAIL}")
        logger.info("Press Ctrl+C to stop")
        
        # Main processing loop
        while True:
            process_emails(gmail_client, ai_engine)
            logger.info(f"Waiting {response_delay} seconds before next check...")
